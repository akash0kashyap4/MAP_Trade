...
async def get_trades(days: int = 30, completed_only: bool = False) -> list:
    ...
    async with pool.acquire() as db:
        ...
        query = f"SELECT * FROM trades WHERE CAST(entry_time AS TIMESTAMP) >= NOW() - INTERVAL '{days} days' {extra} ORDER BY entry_time"
...
async def save_learning_rules(rules: dict, stats: dict):
    ...
    async with pool.acquire() as db:
        await db.execute(
            "INSERT INTO learning_rules (updated_at,rules,win_rate,sharpe,version) VALUES ($1,$2,$3,$4,$5)",
            _now_ist(),
            json.dumps(rules),
